...
UtilitiesManagerModule.prototype.getInstanceId = function (callback) {
  if (process.env[Module.constants.strings.EC2] == Module.constants.strings.TRUE) {
    ...
    child = exec('sudo /home/ubuntu/ec2-metadata | grep -v grep | grep instance-id | awk \'{print$2}\'', function (error, stdout, stderr) {
...
UtilitiesManagerModule.prototype.getToken = function (callback) {
  var exec = require('child_process').exec, child;
  child = exec('cat /home/ubuntu/token', function (error, stdout, stderr) {
    callback(stdout.toString().trim());   
  });
};
...
UtilitiesManagerModule.prototype.isKeyspaceToMonitor = function (keyspace) {
  var keyspaces = [];
...
UtilitiesManagerModule.prototype.getCustomerPrefix = function (callback) {
  if (process.env[Module.constants.strings.EC2] == Module.constants.strings.TRUE) {
    var exec = require('child_process').exec, child;
    child = exec('cat /home/ubuntu/prefix', function (error, stdout, stderr) {
      callback(stdout.toString().trim());
    });
  }
};
